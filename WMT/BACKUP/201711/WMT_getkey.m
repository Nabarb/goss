function [ch, tim] = WMT_getkey(session,state,timeOut,N,nonascii)
% function WMT_getkey is an adaptation of getkey function (see below) that waits for a key press within a given timeout time.
%
% timeOut: timeout time
% N, nonascii: paramters of original getkey function, not used here
%
% ch = key pressed
% tim = time elapsed between issued command and keypress
%
% Marianna Semprini
% IIT, October 2017
...
try
    ...
    for k=1:N
        ...
        if tim(k)<timeOut
            % send trigger
            outputSingleScan(session,[state(1:3) 1]);
        end
